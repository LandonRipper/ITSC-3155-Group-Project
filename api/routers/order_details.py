...
@router.get("/{order_id}", response_model=schema.OrderDetail)
def read_one(order_id: int, db: Session = Depends(get_db)):
    return controller.read_one(db, order_id=order_id)


@router.put("/{order_id}", response_model=schema.OrderDetail)
def update(order_id: int, request: schema.OrderDetailUpdate, db: Session = Depends(get_db)):
