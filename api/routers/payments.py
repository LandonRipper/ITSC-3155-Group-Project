...
@router.post("/", response_model=PaymentCreate, status_code=201)
...
@router.put("/{payment_id}/{promotion_id}", response_model=schema.Payment)
def apply_promo(payment_id: int, promotion_id: int, db: Session = Depends(get_db)):
    updated = controller.apply_promo(db=db, payment_id=payment_id, promotion_id=promotion_id)
    if updated is None:
        raise HTTPException(status_code=404, detail="Payment not found!")
    return updated